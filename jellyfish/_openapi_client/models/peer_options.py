...
class PeerOptions(BaseModel):
    ...
    if TYPE_CHECKING:
        actual_instance: Union[PeerOptionsWebRTC]
    else:
        actual_instance: Any
    one_of_schemas: List[str] = Field(PEEROPTIONS_ONE_OF_SCHEMAS, const=True)
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> PeerOptions:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> PeerOptions:
        """Returns the object represented by the json string"""
        instance = PeerOptions.construct()
        error_messages = []
        match = 0

        # deserialize data into PeerOptionsWebRTC
        try:
            instance.actual_instance = PeerOptionsWebRTC.from_json(json_str)
            match += 1
