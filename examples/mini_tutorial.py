...
from jellyfish import Notifier, RoomApi, ComponentOptionsHLS, ComponentOptionsHLSSubscribeMode, ComponentOptionsFile
from jellyfish.events import ServerMessageTrackAdded, ServerMessageTrackType, ServerMessageHlsPlayable
...
async def test_notifier():
    ...
    # Create a room to trigger a server notification with h264 as a codec, that allow to use HLS.
    address, room = room_api.create_room(video_codec='h264')

    # Create new room api with returned jellyfish address as a room could be created 
    # on a different jellyfish instance (if you communicate with a cluster of jellyfishes)
    room_api = RoomApi(server_address=address)

    # Add HLS componen manual
    hls_component = room_api.add_component(room.id, ComponentOptionsHLS(subscribe_mode=ComponentOptionsHLSSubscribeMode.MANUAL))
